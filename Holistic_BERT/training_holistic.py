...
def creating_train_set(path):
    ...
    '''
    train_dirs = [f for f in os.listdir(path) if f.find("train")==0] #All the training data
    dataframes = []
    for t in train_dirs: #delete the slicing part
    ...
    train_text = np.array(list(concat_dataframe["tokenized"]))
    attention_masks = np.where(train_text>0, 1, 0)
    train_labels = np.array(list(concat_dataframe["is_deception"]))
...
def train(model, train_dataloader, epochs, evaluation, saved_path, device, optimizer, scheduler):
    ...
    for epoch_i in range(epochs):
        ...
        if evaluation == True:
            ...
            print("-"*70)
        print("\n")
    path = saved_path + "bert_holistic_epoch_" + str(epochs) + ".pth"
